...
class BattleshipGame {
    ...
    setupEventListeners() {
        ...
        if (rotateBtn) rotateBtn.addEventListener('click', () => this.rotateShip());
        if (randomBtn) randomBtn.addEventListener('click', () => this.randomPlacement());
    ...
    startGame() {
        ...
        if (gamePhase) gamePhase.style.display = 'block';
    ...
    updateShipsCount() {
        const playerCount = document.getElementById('player-ships-count');
        const computerCount = document.getElementById('computer-ships-count');
        
        if (playerCount) playerCount.textContent = this.playerShipsRemaining;
        if (computerCount) computerCount.textContent = this.computerShipsRemaining;
    }
    ...
    endGame(winner) {
        ...
        if (winner === 'player') {
            if (resultElement) resultElement.textContent = '🎉 VICTORY! 🎉';
            if (messageElement) messageElement.textContent = 'Congratulations! You have defeated the enemy fleet!';
        } else {
            if (resultElement) resultElement.textContent = '💥 DEFEAT 💥';
            if (messageElement) messageElement.textContent = 'Your fleet has been destroyed. Better luck next time!';
        }
